...
<span class="category_heading"><?php printf( __( 'Search Results for: %s', 'newstheme' ), '<span>' . get_search_query() . '</span>' ); ?></span>
<?php if (have_posts()) : ?>
	<?php while (have_posts()) : the_post(); ?>
	
<div class="category_posts">
<h2><a href="<?php the_permalink() ?>" rel="bookmark" title="<?php the_title_attribute(); ?>"><?php the_title(); ?></a></h2>
<div class="post_dets"><div class="post_author"><span>Written by:</span><span class="post_details"><?php the_author_posts_link(); ?></span></div><div class="post_time"><span>Written on:</span><span class="post_details"><a href="<?php echo get_day_link(get_the_time('Y'), get_the_time('m'), get_the_time('d')); ?>"><?php the_time('F j, Y') ?>
 </a></span></div>
   <?php if (!empty($post->post_password)) { ?><?php }else { ?>
 <?php if ( comments_open() ) : ?><div class="comments-counter"><?php comments_popup_link('0', '1', '%', '', ''); ?></div><?php ; else : ?><div class="comments-off">Comments <br />are closed</div><?php endif; ?><?php } ?></div>
