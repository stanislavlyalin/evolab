...
elseif ( is_single() || is_page() ) {
  ...
  wp_title( '', true );
    ...
    <link rel="shortcut icon" href="<?php esc_url( bloginfo( 'stylesheet_directory' ) ); ?>/favicon.ico" />
        ...
        <?php
        // Retrieve Simply Works Theme Options Data
        global $swc_options;
        $swc_options = get_option( 'swc_theme_options' );
        if ( isset( $swc_options['swc_sidebar'] ) && ($swc_options['swc_sidebar'] == "1") ) {
          echo "<style type=\"text/css\">#contentarea {float: right; border: 0;}</style>";
        }
    ...
    <div id="webpage"><!-- START webpage ID -->
      <div id="header"><!-- START header ID -->
        <div class="wrapper" <?php
          ...
          <a href="<?php echo get_site_url(); ?>"><img style="float: left; padding: 15px 20px 0px 0px" src="<?php echo get_site_url(); ?>/wp-content/uploads/2013/01/Logo_small2.png"/></a>
...
<?php if ( isset( $swc_options['swc_logo'] ) && ($swc_options['swc_logo'] != "") ) {
  ...
  if ( $is_header_image != NULL ) {
    echo "style=\"padding: 30px 0 0 0;\" ";
  }
  ?>><a href="<?php echo home_url(); ?>"><img src="<?php echo esc_attr( strip_tags( $swc_options['swc_logo'] ) ); ?>" alt="<?php esc_attr( strip_tags( bloginfo( 'name' ) ) ); ?>"  /></a></div>
      <?php }
      else { ?>
...
<?php
// Menus only show if user create a menu in Appearence > Menu
